...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  logger=logging.getLogger('run-predict')
  handler=logging.FileHandler('run_information/run-predict.txt')
  ...
  for task_id in range(iteration):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    save_index = sorted(range(len(result_list)), key=lambda k: result_list[k], reverse=True)[1:-1]    # 去掉最小最大值
    ...
    save_configs.to_csv('/home/hmj/tuning_spark/target/target_spark/data/running/sql/aggregation_causal_cbo_p_samples.csv', mode= 'a', header= False, index = False)
    # clean temp data
    apperf_df = pd.DataFrame(columns=apperf_columns)
    hdperf_df = pd.DataFrame(columns=hdperf_columns)
    ##
