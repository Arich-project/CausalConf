# 实验3：用dataszie-aware后，只跑一个causal(pc)
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  input_datasize = 1000000+120000#1000+120#   #5000000 #94000000 #20000 #
  dataszie_values_list = {'wordcount': [800000000, 6400000000],
                    ...
                    'scan':[1000+120, 100000000+12000000]}
  ...
  causal_models = ['pc'] # ['pc', 'fci', 'mmhc']  # , 'ges'
  ...
  for task_id in range(causal_id):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    if task_id == 0:
        ...
        if min_distance == 0:  # 输入的是相同datasize
            ...
            ds_samples[current_ds_index] = pd.concat([ds_samples[current_ds_index],
                                                      new_samples_df], axis=0, ignore_index=True)
            ...
            # draw_graph(current_edges,'{}_{}'.format(current_ds_index, model_name))  # draw updata model
        ...
        G[model_name], graph_edges[model_name], paths[model_name], current_tune_conf[model_name] = get_path_columns(samples_df, ds_samples,
                                                                                        datasize_num, similar_ds_list,ds_G[model_name], all_paths[model_name],
                                                                                        app_setting, current_columns, obj_columns, NUM_PATHS)
    ...
    # 获取干预数据  #第二次更新causal model时，未更新的model不改变data_x_list, data_y_list变量的值    #
    ...
    # cbo iteration #
    cbo_iteration = 20 # len(causal_models) * 3 + 1  #  3  #
        ...
        for rep in range(test_config.optimizer.repitition):
            ...
            for name in ['spark_memory_storageFraction', 'spark_memory_fraction', 'spark_shuffle_sort_bypassMergeThreshold', 'jvm_max_tenuringThreshold', 'jvm_survivor_ratio', 'spark_shuffle_spill_compress', 'spark_executor_cores', 'spark_kryoserializer_buffer', 'spark_storage_replication_proactive', 'spark_executor_memory', 'jvm_gc_timeratio', 'spark_speculation', 'jvm_max_gc_pause', 'spark_locality_wait', 'spark_driver_memory', 'spark_default_parallelism', 'jvm_par_gc_thread', 'spark_shuffle_compress']:
                #['spark_broadcast_blockSize', 'spark_task_maxFailures', 'spark_rdd_compress', 'spark_memory_fraction', 'spark_executor_cores', 'spark_speculation', 'spark_shuffle_sort_bypassMergeThreshold', 'spark_storage_replication_proactive', 'spark_driver_memory', 'spark_scheduler_revive_interval', 'jvm_max_gc_pause']:
                   #  ['jvm_new_ratio', 'spark_executor_cores', 'spark_memory_offHeap_size', 'spark_storage_replication_proactive',
