...
def update_BO_models(mean_function, var_function, data_x, data_y, Causal_prior):
    ...
    if Causal_prior==False:
        ...
        mf.f = lambda x: mean_function(x)       # 重定义GP的均值函数
        mf.update_gradients = lambda a, b: None
        causal_kernel = CausalRBF(data_x.shape[1], variance_adjustment=var_function,   # 重定义GP的方差函数
                                            lengthscale=1., variance=1., ARD = True)
