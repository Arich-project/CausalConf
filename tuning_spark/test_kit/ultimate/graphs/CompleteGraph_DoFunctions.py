...
def compute_do(observational_samples, function, value, manipulative_variables, variables):
    # function = function["compute_function"]
    for i in range(len(variables)):
        observational_samples[variables[i]] = value[i]

    intervened_inputs = observational_samples[manipulative_variables].values
    mean_do = np.mean(function.predict(intervened_inputs)[0])
    var_do = np.mean(function.predict(intervened_inputs)[1])
