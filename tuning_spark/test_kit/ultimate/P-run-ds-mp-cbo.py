# 实验4：用一个dataszie的初始化数据跑其他datasize，并且只用一个causal算法
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  # datasize variable
  level_ds_samples_num = 20000 # 132
  datasize_list = {'wordcount': [3200000000], # [1600000000, 2400000000, 3200000000, 4800000000, 6400000000],     # slarge,llarge,large,shuge,lhuge
                   'aggregation:': [15000000+1800000],
                   'bayes': [400000],   # [200000, 350000, 500000, 600000, 700000]}
                   'join':[1000000+120000]}  #
  ...
  samples_df = pd.read_csv('/home/hmj/tuning_spark/target/target_spark/data/initial/sql/scan_lhs_samples-2000.csv') #Z_scan_samples_single_150
  ...
  for task_id in range(causal_id):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    for bo_id in range(cbo_iteration):
        ...
        # 存储未去除值重复的中间变量的数据
        save_configs.to_csv('/home/hmj/tuning_spark/target/target_spark/data/running/sql/scan_causal_cbo_p_samples.csv', mode='a', header=False, index=False)
        # clean temp data
        apperf_df = pd.DataFrame(columns=apperf_columns)
        hdperf_df = pd.DataFrame(columns=hdperf_columns)
        ##
