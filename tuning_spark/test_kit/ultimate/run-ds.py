# 实验2：用single dataszie的初始化数据跑另一个datasize
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  NUM_PATHS = 8
  query = "best"
  input_workload = 'bayes' # 'bayes' #
  input_datasize = 200000 #250000 #
  ...
  # datasize variable
  level_ds_samples_num = 200#150  # 50   #
  # ds_samples_num = 50
  datasize_list = {'terasort':[15000000],
                   'wordcount': [3200000000], # [1600000000, 2400000000, 3200000000, 4800000000, 6400000000],     # slarge,llarge,large,shuge,lhuge
                   'aggregation:': [50000+6000, 100000+12000, 500000+60000, 1000000+120000, 20000000+2400000],
                   'bayes': [600000]}   # [200000, 350000, 500000, 600000, 700000]}
  ...
  for task_id in range(causal_id):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    if task_id == 0:
        ...
        for j in range(len(causal_models)):
            ...
            for ds_index in range(0, datasize_num):
                ...
                current_G, ds_G_paths, current_edges, current_time = run_updata_model(CM, ds_samples[ds_index], app_setting, tabu_edges, current_columns,   # 单datasize容易造成新数据更新因果图无法识别因果关系
                                                                        obj_columns, NUM_PATHS, model_name, causal_models, task_id)
    ...
    # cbo iteration #
    cbo_iteration = len(causal_models) * 3 + 1   #  = 22
