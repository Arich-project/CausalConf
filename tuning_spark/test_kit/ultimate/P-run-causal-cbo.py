# 实验1：datasize-aware + model pool + cbo
    ...
    for path in paths:
        for i in range(len(path)):
            if i > 0:
                if (path[i],path[i-1]) not in G_di_edges:
                    G_di_edges.append((path[i],path[i-1]))
    G_di_edges = get_resolve_edges(current_columns, G_di_edges, obj_columns[0])  # without loop edges
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  NUM_PATHS = 25
  # query = "best"
  input_workload = 'join' #'terasort' #
  input_datasize = 1000000+120000 #5000000 #94000000 #20000 #
  dataszie_values_list = {'wordcount': [800000000, 6400000000],
                    'terasort': [3200, 640000000],
                    'aggregation:': [50000+6000, 20000000+2400000],
                    'bayes': [100000, 2000000],
                    'join': [1000+120, 100000000+12000000]}
  ...
  datasize_list = {'terasort':[32000,320000,3200000,32000000,64000000],  # 3200000,
                   'wordcount': [1600000000, 2400000000, 3200000000, 4800000000, 6400000000],     # slarge,llarge,large,shuge,lhuge
                   'aggregation:': [50000+6000, 100000+12000, 500000+60000, 1000000+120000, 20000000+2400000],
                   'bayes': [200000, 300000, 400000, 500000, 600000],
                   'join':[1000+120, 100000+12000, 1000000+120000, 10000000+1200000, 100000000+12000000]}  #
  ...
  ## hmj ##
  # read initialize data (without os_columns)  # extract observer data from csv_file use pandas
  samples_df = pd.read_csv('/home/hmj/tuning_spark/target/target_spark/data/initial/sql/scan_p_samples_30.csv')  #
  ...
  # model = BaggingRegressor(n_estimators=100, random_state=42).fit(X_train, y_train)
  #

  # causal model pool type
  causal_models = ['pc','fci','mmhc']  # , 'ges'
  ...
  num_interventions = 5 #10   # 并不准确，适当减少数量
  type_cost = 1
  num_trials = 2  # 50
  causal_prior = True
  ...
  task = 'min'
  # 根据策略取一定数量的观察数据   # 根据datasize分层选择   # test_samples = ds_samples.copy()
  ...
  for task_id in range(causal_id):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    if task_id == 0:
        ...
        for j in range(len(causal_models)):
            ...
            for ds_index in range(0, datasize_num):
                ...
                # draw_graph(current_edges, '{}_{}_{}'.format(model_name, task_id, ds_index))  # 保存为图片, 命名为 taskid_dsindex_modelname
        ...
        if min_distance == 0:  # 输入的是相同datasize
            ...
            print("======  UPDATA CAUSAL MODEL DATASIZE_INDEX: ", current_ds_index, "======")    # 单datasize容易造成新数据更新因果图无法识别因果关系
            ...
            # draw_graph(current_edges,'{}_{}'.format(model_name, current_ds_index))  # draw updata model
        ...
        # ds_samples数据已经更新，重新组合该dataszie下的因果图模型和paths                 # task_id==1时是否更新全部，以及是否只使用最新的数据更新
        G[model_name], graph_edges[model_name], paths[model_name], current_tune_conf[model_name] = get_path_columns(samples_df, ds_samples.copy(),
                                                                                        datasize_num, similar_ds_list, ds_G[model_name], all_paths[model_name],
                                                                                        app_setting, current_columns, obj_columns, NUM_PATHS)
    ...
    # samples_file_path = "/home/hmj/tuning_spark/target/target_spark/data/initial/sql/join_lhs_samples-2000.csv"
    # Samples = pd.read_csv(samples_file_path, header=0, encoding="gbk")  # 读取建模用的样本数据
    # get_divide_model(Samples, save_columns, unique_tune_configs, apperf_columns, obj_columns)
    ...
    # alpha_coverage, hull_obs, coverage_total = 1, 2, 3     # 用于观察-干预权衡的greedy策略,可改成 预测-实际的相对误差 或 实际-最小值/默认值的目标改进
    # 获取干预数据  #第二次更新causal model时，未更新的model不改变data_x_list, data_y_list变量的值    # problems：即使跑了10轮，并且 observational_samples 也更新了，但第二次并未完全利用新的数据
    ...
    # cbo iteration                            #  改：初始运行10次,然后就更新causal model,之后每次运行20次
    cbo_iteration = 20 # len(causal_models) * 3 + 1  #  = 22
    for bo_id in range(cbo_iteration):
        ...
        for rep in range(test_config.optimizer.repitition):
            ...
            # )
            #
            # result_list.append(- result)            # result_list.append(round(random.uniform(-160, -500), 3))

            # model predict
            filename = './predict_models/Z_finalized_model.sav'  # 模型名称
            model = pickle.load(open(filename, 'rb'))  # 载入离线模型
            # Samples = []
            Samples = [input_datasize]
            for name in ['jvm_new_ratio', 'spark_executor_cores', 'spark_memory_offHeap_size', 'spark_storage_replication_proactive',
                ...
                # ['spark_storage_replication_proactive', 'spark_executor_memory', 'spark_storage_memoryMapThreshold', 'spark_task_maxFailures', 'spark_driver_memory', 'spark_memory_fraction', 'spark_shuffle_sort_bypassMergeThreshold', 'spark_speculation', 'spark_executor_cores', 'spark_shuffle_file_buffer', 'spark_io_compression_codec', 'jvm_max_tenuringThreshold','spark_memory_offHeap_size']:#['spark_broadcast_blockSize', 'spark_task_maxFailures', 'spark_rdd_compress', 'spark_memory_fraction', 'spark_executor_cores', 'spark_speculation', 'spark_shuffle_sort_bypassMergeThreshold', 'spark_storage_replication_proactive', 'spark_driver_memory', 'spark_scheduler_revive_interval', 'jvm_max_gc_pause']:#
        ...
        if task_id == 0 and bo_id == 0:
            ...
            if causal_models[index] in before_execute_opty.keys() and (-metric_result) <= min(before_execute_opty[causal_models[index]]) and import_rate < -0.2:
                # 添加相对于自己有改进的           # 并且对于默认配置有改进的 # and (-metric_result)-observational_samples.iloc[0, -1] < -0.2
            ...
            if set(causal_models) == set(model_execute_opty.keys()) and not first_best_index:
                ...
                # if min_distance != 0 :
                #     current_default_opty =    # 新的dataszie根据实际运行更改改进目标
        ...
        # 存储未去除值重复的中间变量的数据
        save_configs.to_csv('/home/hmj/tuning_spark/target/target_spark/data/running/micro/terasort_causal_cbo_p_samples.csv', mode='a', header=False, index=False)
        # clean temp data
        apperf_df = pd.DataFrame(columns=apperf_columns)
        hdperf_df = pd.DataFrame(columns=hdperf_columns)
        ##
