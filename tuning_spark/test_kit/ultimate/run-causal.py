...
def run_fci_loop1(CM, df, tabu_edges, columns, obj_columns,
                     NUM_PATHS):
    ...
    G_edges = di_edges + bi_edges
    G_di_edges = get_resolve_edges(columns, G_edges, obj_columns[0])  # without loop edges
    edges_num = len(di_edges)
    if len(di_edges) > len(G_di_edges):
        edges_num = len(G_di_edges)
    di_edges = G_di_edges[:edges_num]
    bi_edges = G_di_edges[edges_num:]
...
def run_mmhc_loop(df, tabu_edges, columns, obj_columns):
    ...
    mmhc_results = MMHC(df, alpha= 0.02, score_function='bic')  # , score_function='bic'
...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  samples_df = pd.read_csv('/home/hmj/tuning_spark/target/target_spark/data/initial/ml/bayes_samples_single_150.csv')  #sql/join_lhs_samples-2000.csv bayes_samples_single_150，extract data from csv_file use pandas
  ...
  # samples_df.to_csv('/home/hmj/tuning_spark/target/target_spark/data/initial/ml/bayes_samples_single_150.csv', mode='a',header=False, index=False)
  running_df = pd.read_csv('/home/hmj/tuning_spark/target/target_spark/data/initial/ml/bayes_samples_cameo_30.csv')
  ...
  samples_df = samples_df[current_columns]  # save current columns values
  for apcolumn in app_columns:
      if apcolumn not in current_columns:
          app_columns.remove(apcolumn)
  ...
  # fci: build
  G, di_edges, bi_edges = run_fci_loop1(CM, samples_df, tabu_edges,
                                           current_columns, obj_columns, NUM_PATHS)
  # print("constructed model time:{}".format(time.time()-graph_start))
  ##
  # ges: build
  ...
  graph_build_alltime = graph_build_alltime + graph_end
  ...
  iteration = 40
  for task_id in range(iteration):
    B_start = time.time()
    ...
    ref_index = samples_df[[obj_columns[0]]][1:149].idxmin()
    ref_df = samples_df.loc[ref_index]
    ref = ref_df.iloc[0]
    previous_config = ref[app_columns].copy()

    # identify causal paths
    paths = CM.get_causal_paths(current_columns, di_edges + bi_edges,
                                obj_columns)
    ...
    if len(obj_columns) < 2:
        ...
        if task_id > 0:
            recommond_config = CM.compute_individual_treatment_effect(samples_df, paths,
    ...
    save_configs.to_csv('/home/hmj/tuning_spark/target/target_spark/data/running/ml/bayes_causal_samples.csv', mode= 'a', header= False, index = False)
    # clean temp data
    temp_df = pd.DataFrame(columns=perf_columns)
    mean_df = pd.DataFrame(columns=perf_columns)
    ...
    logger.info('the time of evaluate configs {} is {}'.format(task_id, round(C_end,2)))
