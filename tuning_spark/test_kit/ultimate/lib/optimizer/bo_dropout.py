...
class BOdropoutOptimizer():
  ...
  def get_conf(self, current_ob_x, current_ob_y):
    x_view = self.bod.bo.space.X
    y_view = self.bod.bo.space.Y
    length = self.bod.bo.space._length
    if length >= 10:
        number = int(length / 4)
        ...
        for i in choose_number:
            ...
            print('see the value of y')
            print(y_view[i])
            print('see the value of y_view')
            print(y_view)
        for x, y in zip(pseudo_point_x, pseudo_point_y):
            for i in range(len(x)):
                x[i] = uniform(x[i] - 0.1, x[i] + 0.1)
            self.add_observation((x, y))
    configd = self.bod.get_conf()
    self.bod.bo.space._length = length
    self.bod.bo.space._Xarr = x_view
    self.bod.bo.space._Yarr = y_view
    self.bod.bo.space._Xview= x_view
    self.bod.bo.space._Yview= y_view
    ...
    if dropout_fill == 'rand':
        ...
        #set the value of keys_d, need scale (-1�?) to the original scale
