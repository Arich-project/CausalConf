...
async def main(test_config,os_setting,app_setting,tune_conf):
  ...
  one_sample = [5,2,107,0.9,0.3,0,0,1,1,14,0,1,74,1,4,120,173,2,4,713,126,24,30,2,92,406,15,1,6,1]
  # #
  # specific_samples = []
  ...
  for task_id in range(iteration):#tqdm(range(test_config.optimizer.iter_limit)):
    ...
    if task_id == 0:
        ...
        sampled_config = translate_array_value(one_sample, app_setting)
    ...
    for rep in range(test_config.optimizer.repitition):
        ...
        # result_list.append(round(random.uniform(-110, -200), 3))
